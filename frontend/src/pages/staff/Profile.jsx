...
import {
  User, Mail, Briefcase, Lock, LogOut, Camera,
  Eye, EyeOff, ChevronRight, Check, X,
} from 'lucide-react'
  ...
  return (
    <AppLayout role="staff">
      ...
      <div className="card mb-6 flex flex-col sm:flex-row items-center gap-5 py-6">
        ...
        <div>
          ...
          <span className="inline-block mt-2 bg-primary-50 text-primary-700 text-xs font-bold px-3 py-1 rounded-full">
            Staff Member &bull; {user?.department || 'General'}
          </span>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
          ...
          {editing ? (
            ...
            <div className="space-y-4">
              {[
                ...
                { icon: Briefcase, label: 'Department', value: user?.department || 'General' },
                { icon: User,      label: 'Staff ID',   value: user?.staff_id || 'Not set' },
        ...
        {/* Settings — only Change Password */}
        <div className="card">
          <h3 className="font-bold text-gray-800 mb-3">Account Settings</h3>
          <div className="divide-y divide-gray-50">
            <button
              onClick={() => setChangingPwd(!changingPwd)}
              className="w-full flex items-center justify-between py-3.5 hover:bg-gray-50 rounded-xl px-2 transition-colors"
            >
